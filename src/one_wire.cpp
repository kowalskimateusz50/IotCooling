...
bool One_wire::reset_check_for_device() const {
    ...
    timer_us_initialize(480); // wait 480us
    while (!timer_us_elapsed()){}

	gpio_set_dir(_data_pin, GPIO_IN); // let the data line float high

    timer_us_initialize(70);	// wait 70us
    while (!timer_us_elapsed()){}
    ...
    timer_us_initialize(410);	// wait 410us
    while (!timer_us_elapsed()){}
...
void One_wire::onewire_bit_out(bool bit_data) const {
    ...
    timer_us_initialize(3);	// wait 3us
    while (!timer_us_elapsed()){}

	if (bit_data) {
		gpio_put(_data_pin, true);

    	timer_us_initialize(55);	// wait 55us
    	while (!timer_us_elapsed()){}

	} else {

    		timer_us_initialize(60);	// wait 60us
    		while (!timer_us_elapsed()){}

			gpio_put(_data_pin, true);

    		timer_us_initialize(5);	// wait 5us
    		while (!timer_us_elapsed()){}
	}
}
...
bool One_wire::onewire_bit_in() const {
    ...
    timer_us_initialize(45);	// wait 45us
    while (!timer_us_elapsed()){}
